...
afterEach(function (): void {
    collect($this->tempFiles)->each(fn ($path) => @unlink($path));
});

it('captures a download with fluent assertions', function (): void {
    Route::get('/', fn () => '<a href="/file" download="report.pdf">Download</a>');
    Route::get('/file', fn () => downloadResponse('Hello!', 'report.pdf'));
...
it('saves a download to disk', function (): void {
    Route::get('/', fn () => '<a href="/file" download="data.txt">Download</a>');
    Route::get('/file', fn () => downloadResponse('File contents here', 'data.txt'));
...
it('retrieves the temporary download path', function (): void {
    Route::get('/', fn () => '<a href="/file" download="temp.bin">Download</a>');
    Route::get('/file', fn () => downloadResponse('content', 'temp.bin'));
...
it('handles JavaScript-triggered downloads', function (): void {
    ...
    ');
    Route::get('/file', fn () => downloadResponse('JS Download!', 'dynamic.txt'));

    visit('/')
        ->expectDownload(fn ($page) => $page->click('#btn'))
        ->assertFilename('dynamic.txt')
        ->assertSuccessful();
});

it('handles binary file downloads', function (): void {
    Route::get('/', fn () => '<a href="/image" download="pixel.png">Download</a>');
...
it('reads download contents directly', function (): void {
    Route::get('/', fn () => '<a href="/file" download="data.txt">Download</a>');
    Route::get('/file', fn () => downloadResponse('Hello, World!', 'data.txt'));
...
it('asserts filename contains', function (): void {
    Route::get('/', fn () => '<a href="/file" download="invoice-2024-001.pdf">Download</a>');
    Route::get('/file', fn () => downloadResponse('PDF content', 'invoice-2024-001.pdf'));
...
it('asserts content contains', function (): void {
    Route::get('/', fn () => '<a href="/file" download="report.csv">Download</a>');
    Route::get('/file', fn () => downloadResponse("Name,Email\nJohn,john@example.com", 'report.csv'));
...
it('chains assertions fluidly', function (): void {
    Route::get('/', fn () => '<a href="/file" download="chain.txt">Download</a>');
    Route::get('/file', fn () => downloadResponse('Fluent!', 'chain.txt'));
...
it('captures multiple downloads as a collection', function (): void {
    ...
    ');
    Route::get('/file/{name}', fn ($name) => downloadResponse("Content: {$name}", $name));

    $downloads = visit('/')->expectDownloads(
        fn ($page) => $page->click('#all'),
        count: 3
    );
...
it('saves multiple downloads with collection methods', function (): void {
    ...
    ');
    Route::get('/file/{name}', fn ($name) => downloadResponse("File: {$name}", $name));
